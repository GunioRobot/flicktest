...
int insert_proc(MYSQL *conn, int i, int nu_bulk)
{
  ...
  else
  {
    ...
    while (j < 128)
    {
      ...
      sprintf(query, "INSERT INTO `IPD_benchmark`.`proc` (`clock`, `hostname`, `pid`"
              ", `pname`, `cpuper`, `ioper`, `mem`, `fd`, `rt`, `prio`) VALUES "
              "(%lu, 'dadvmn0433%u0433', %u, 'init', '20', '10', '5', '45', '35',"
              " '90')", t1+j, i, k);
      k--;
      while(--k)
      {
...
int insert_disk(MYSQL *conn, int i, int nu_bulk)
{
  ...
  if (nu_bulk == 0)
  {
    ...
    {
      time(&t1);
      sprintf(query, "INSERT INTO `IPD_benchmark`.`disk` (`clock`, `hostname`,"
  ...
  else
  {
    ...
    while (j < 128)
    {
      ...
      sprintf(query, "INSERT INTO `IPD_benchmark`.`disk` (`clock`, `hostname`,"
              " `diskid`, `diskname`, `ior`, `iowr`, `iowait`, `qlen`, "
              "`total_size`, `occupied`) VALUES (%lu, 'dadvmn0433%u3', %u,"
              " 'sda1', '5', '6', '3', '7', '20000', '5000')", t1+j, i, k);
      k--;
      while(--k)
      {
        sprintf(temp_query, ",(%lu, 'dadvmn0433%u', %u,"
              " 'sda1', '5', '6', '3', '7', '20000', '5000')",t1+j, i, k);
        strcat(query, temp_query);
      }
      strcat(query, ";");
...
int insert_nic(MYSQL *conn, int i, int nu_bulk)
{
  ...
  if (nu_bulk == 0)
  {
    ...
    {
      time(&t1);
      sprintf(query, "INSERT INTO `IPD_benchmark`.`NIC` (`hostname`, `clock`,"
...
int insert_fs(MYSQL *conn, int i, int nu_bulk)
{
  ...
  if (nu_bulk == 0)
  {
    ...
    {
      time(&t1);
      sprintf(query, "INSERT INTO `IPD_benchmark`.`FS` (`hostname`, `clock`, `fsid`,"
...
init_db(MYSQL **conn)
{
  ...
  char *server = "10.176.226.226";
  char *user = "root";
  char *password = "iiitc_Z01"; /* set me first */
  char *database = "IPD_benchmark";
